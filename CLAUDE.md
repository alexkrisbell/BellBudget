...
* **Dashboard**: at-a-glance monthly snapshot — budget overview, category progress, income summary, streak status, net worth (current value + recent change, click to expand a trend chart inline). Deliberately kept lean; features get added here only when they're a daily-glance concern. Net worth moved here from Stats once investment tracking made it a number worth checking daily instead of a periodic insight — recurring bills stayed on Stats, since "what's coming up" is a Bills-page concern, not a daily one.
* **Stats & Insights**: full net worth history (assets minus liabilities, including investment balances), monthly saved/spent/savings-rate trends (click a stat tile to reveal its chart, one at a time, instead of always rendering all of them), recurring bill detection (flags subscriptions from transaction patterns), investment holdings (per-account, collapsed by default — click an account to see its positions).
...
✅ Done, though real usage bent the original plan: investment holdings live on the Stats page (per-account, collapsed by default) rather than a separate nav item, so a net worth trend and an investment balance stayed one destination as intended. But net worth itself turned out to be a *daily* check once investments made it a bigger, more complete number — not the periodic insight it was when it was just bank accounts — so its quick-glance view moved to the Dashboard instead (see §5), with the Stats page keeping the full history/detail view. Worth remembering: real usage overruling an earlier assumption here is the Feature Test (§4) working as intended, not scope creep.
